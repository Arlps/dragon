...
function(window) {
	...
	function c(c) {
		...
		document.oncontextmenu = function() {
			...
			if (null == a) throw new Error("Stage.remove³ö´í: ²ÎÊýÎªnull!");
		...
		this.initialize = function(c) {
			...
			if (null != this.alarm && "" != this.alarm) {
				var b = this.alarmColor || "255,0,0",
					c = this.alarmAlpha || .5;
				a.beginPath(), a.font = this.alarmFont || "10px Î¢ÈíÑÅºÚ";
			...
			if (null == b) throw new Error("Node.setImage(): ²ÎÊýImage¶ÔÏóÎª¿Õ!");
			var d = this;
			if ("string" == typeof b) {
				var e = j[b];
	...
	function c(c, d) {
		null == d && (d = {}); {
			var e = d.cx,
				...
				i = d.hScale || 1,
				j = d.vScale || 1;
			d.beginAngle || 0, d.endAngle || 0.5* Math.PI
		}
		if (null == e || null == f) {
			var k = b(c);
			e = k.x, f = k.y
		...
		return b.radius = 150, b.colors = ["#3666B0", "#2CA8E0", "#77D1F6"], b.datas = [.3, .3, .4], b.titles = ["A", "B", "C"], b.paint = function(a) {
			...
			for (var e = 0, f = 0; f < this.datas.length; f++) {
				...
				var h = this.titles[f] + ": " + (100 * this.datas[f]).toFixed(2) + "%",
					...
					j = (a.measureText("Ìï").width, (e + e + g) / 2),
