...
TreeGrid = function(_config){
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			var trid = this.trid || this.getAttribute("trid");console.log(this.getAttribute("trid"))

			var isOpen = __root.find("#" + trid).attr("open");//isOpen==open
			isOpen = (isOpen == "Y") ? "N" : "Y";console.log(isOpen)//isOpen==Y
			__root.find("#" + trid).attr("open", isOpen);console.log(__root.find("#" + trid).attr("open")); console.log(isOpen)//attr(open)==open
			__root.find("#" + trid).attr("open1", isOpen);console.log(__root.find("#" + trid).attr("open1"));//attr(open1)==Y
			__root.find("#" + trid).attr("open1", "哈哈");console.log(__root.find("#" + trid).attr("open1"));//attr(open)==哈哈
	...
	showSubs = function(_trid){
		var isOpen = __root.find("#" + _trid).attr("open");console.log(isOpen)
		if(isOpen == "Y"){
